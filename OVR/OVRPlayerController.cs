...
public class OVRPlayerController : MonoBehaviour {
    ...
    /// <summary>
    /// [Custom parameters] Minimum time between two snap rotations.
    /// </summary>
    [Tooltip("Minimum time between two snap rotations).")]
    public float SnapRotationCooldown = 2.0f;
    private float snapRotationTimer;

    /// <summary>
    /// [Custom parameters] The player will move in fixed steps if Snap Translation is enabled.
    /// </summary>
    [Header("Translation configuration")]
    [Tooltip("The player will move in fixed steps if Snap Translation is enabled.")]
    public bool SnapTranslation = true;

    /// <summary>
    /// [Custom parameters] Translate the player from this distance (when Snap Translation enabled).
    /// </summary>
    [Tooltip("Translate the player from this distance (when SnapTranslation enabled).")]
    public float SnapDistance = 2.0f;
    
    /// <summary>
    /// [Custom parameters] Minimum time between two snap translations.
    /// </summary>
    [Tooltip("Minimum time between two snap translations).")]
    public float SnapTranslationCooldown = 2.0f;
    private float snapTranslationTimer;

    /// <summary>
    /// [Deprecated] When enabled, snap rotation will happen about the guardian rather
    /// than the player/camera viewpoint.
    /// </summary>
    [Header("HMD Behaviour configuration")]
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableRotation)
        {
            ...
            if (SnapRotation)
            {
                if (snapRotationTimer < SnapRotationCooldown) {
        ...
        // [Custom code] Handling SnapTranslation
